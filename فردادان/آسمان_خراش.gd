...
func پیشروی(شماره: int):
	if پارسی‌سازی.شماره_پارسی(شماره) == نمونه‌فرصت‌ها.front().name:
		نمونه‌فرصت‌ها.pop_front().queue_free()
		امتیاز += 1
		$"سر/امتیاز".text = پارسی‌سازی.شماره_پارسی(امتیاز)
...
func نمایش():
	...
	for فرصت in فرصت‌ها:
		...
		نمونه.name = پارسی‌سازی.شماره_پارسی(فرصت + 1)
		نمونه.text = پارسی‌سازی.شماره_پارسی(فرصت + 1)
		نمونه.disabled = true
