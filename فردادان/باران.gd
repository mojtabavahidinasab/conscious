...
func بارش():
	...
	if کار == کارها["جمع"]:
		...
		شماره۱ = str(شماره۱)
		شماره۲ = str(شماره۲)
		نمونه.بروزرسانی(کار, پارسی‌سازی.شماره_پارسی(شماره۱, true), پارسی‌سازی.شماره_پارسی(شماره۲, true), پاسخ, ضریب)
...
func بنویس():
	for هرقطره in قطرات:
		if snapped(float($"تخته‌کلید".نوشتار), .01) == هرقطره.پاسخ:
			$آوا.سردادن("درست")
			بازی.امتیاز += هرقطره.ضریب_امتیاز
	...
	if امتیاز_ضرب == کران_بالای_ضرب - int(کران_بالای_ضرب / 10):
		کران_بالای_ضرب *= 2
	if بازی.امتیاز == int(کران_بالای_جمع / 2):
		کران_بالای_جمع *= 3
	if not بازی.امتیاز % 5 and کران_پایین_تندی_بارش >= 1:
